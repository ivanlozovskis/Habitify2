...
def delete_habit(habit, deleted=None):
    if deleted==None:
    ...
    if habit.habit_id in is_active[is_active['Deleted'].isna()].iloc[:,0].values:
        ...
        return "Either Habit_id is wrong or the Habit was already deleted"


def done(hm, habit, timespent=None, time=None):
    print(habit.name, timespent, time)
    if time == None:
        time = str(date.today())
    ...
    try:
        ...
        pd.to_datetime(time)
        if pd.Timedelta(pd.to_datetime(date.today())- pd.to_datetime(time)) >= pd.Timedelta(0):
...
def longest_streak(habit=None):
    ...
    return int(longest[longest['freq']=='Daily']['max_consecutive_days'].values[0]), int(longest[longest['freq']=='Weekly']['max_consecutive_days'].values[0])
